...
export const appRouter = router({
  ...
  savedSearches: router({
    ...
    toggleNotifications: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return await toggleSavedSearchNotifications(
          input.id,
          ctx.user.id,
          input.enabled
        );
    ...
    triggerNotifications: protectedProcedure.mutation(async ({ ctx }) => {
      // Only allow admin users to trigger notifications
      if (ctx.user.role !== "admin") {
        throw new Error("Unauthorized: Admin access required");
      }
      const { processSavedSearchNotifications } = await import(
        "./jobs/savedSearchNotifications"
      );
  ...
  reports: router({
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { createReport, getListingById } = await import("./db");
